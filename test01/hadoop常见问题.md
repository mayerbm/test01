...
***
- <font color=red>hive表无法删除</font>  
当执行hive表的某个操作被不可控原因中断时,此时该hive表是无法操作的
hive>show locks tablename;  结果是shared/exclusive   说明该表被锁住了
jps查看进程有一些runjar
ps -aux|grep pid找到操作该表的pid
kill -9 pid杀掉该进程,然后该表就解锁了,此时可以删除表
***
